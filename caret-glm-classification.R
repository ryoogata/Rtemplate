...
for(i in 1:(n_repeats * nresampling)) SEEDS[[i]] <- sample.int(1000, 1)
...
# 説明変数一覧の作成
explanation_variable <- myPackage::exclude_variables(STUDY, c("response", "data_id", "period"))
...
# 学習用データ全てを利用してデルを作成
finalModel <- glm(formula = as.formula(paste("response", "~" , paste(explanation_variable, collapse = "+")))
      ,data = STUDY
      ,family = "binomial"
      )
...
if (is.null(model_list[[1]]$preProcess)){
  # preProcess を指定していない場合
  pred_evaluation <- predict(finalModel, EVALUATION, type="response")
  ...
  pred_evaluation <- predict(finalModel, PREPROCESS, type="response")
