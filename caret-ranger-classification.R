...
# variance: regression の場合に利用
# maxstat: regression の場合に利用
...
for(i in 1:(n_repeats * nresampling)) SEEDS[[i]] <- sample.int(1000, length(MTRY) * length(SPLITRULE))
...
finalModel <- ranger(formula = as.formula(paste("response", "~" , paste(explanation_variable, collapse = "+")))
      ,data = STUDY
      ,mtry = model_list[[1]]$bestTune["mtry"] %>%  as.numeric
      ,splitrule = model_list[[1]]$bestTune["splitrule"] %>% transform(splitrule=levels(splitrule)[splitrule]) %>% as.character
      ,min.node.size = model_list[[1]]$bestTune["min.node.size"] %>% as.numeric
      ,importance = "impurity"
      ,probability = TRUE
      )
